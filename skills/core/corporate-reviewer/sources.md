...
claim-based · freshness: conditional. This skill returns `unknown` and does not proceed on a consequential conclusion when a required current source is missing (see `docs/adr` and `GOVERNANCE.md` "Change controls").

## Primary Authorities

- The sources explicitly cited by the reviewed artifact and their registry records.

_No T1 registry entries map to this skill's domain; primary authority is the corporate instrument below._

## Corporate Sources

- Authorized policies, evidence, and applicable approval records.
...
Do not certify a critical claim whose authoritative current source is absent or stale.
